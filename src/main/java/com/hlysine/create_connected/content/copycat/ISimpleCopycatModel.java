...
public interface ISimpleCopycatModel {
     ...
     *
     * @param sourceQuad    The source quad to copy from.
     * @param emitter       The quad emitter to output to.
     * @param spriteFinder  The sprite finder for texture lookup.
     ...
     */
    default void assemblePiece(QuadView sourceQuad, QuadEmitter emitter, SpriteFinder spriteFinder, int angle, boolean flipY, MutableVec3 offset, MutableAABB select, MutableCullFace cull) {
        select.rotate(angle).flipY(flipY);
        offset.rotate(angle).flipY(flipY);
        cull.rotate(angle).flipY(flipY);
        if (cull.isCulled(sourceQuad.lightFace())) {
            return;
        }
        emitter.copyFrom(sourceQuad);
