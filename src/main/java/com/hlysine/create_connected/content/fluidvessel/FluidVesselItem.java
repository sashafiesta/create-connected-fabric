	...
	private void tryMultiPlace(BlockPlaceContext ctx) {
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				((io.github.fabricators_of_create.porting_lib.entity.extensions.EntityExtensions) player).getCustomData()
						.putBoolean("SilenceVesselSound", true);
				super.place(context);
				((io.github.fabricators_of_create.porting_lib.entity.extensions.EntityExtensions) player).getCustomData().remove("SilenceVesselSound");
			}
		}
	}
