...
public class ItemUseOverridesMixin {
    @WrapOperation(
            method = "onBlockActivated(Lnet/minecraft/world/level/Level;Lnet/minecraft/world/entity/player/Player;Lnet/minecraft/world/level/Level;Lnet/minecraft/world/InteractionHand;Lnet/minecraft/world/phys/BlockHitResult;)Lnet/minecraft/world/InteractionResult;",
    ...
    private static InteractionResult preciseHitLocation(BlockState instance,
        ...
        if (PreciseItemUseOverrides.OVERRIDES.contains(id) && blockHitResult.getType() == HitResult.Type.BLOCK) {
            HitResult hitResult = player.pick((player.position().distanceTo(blockHitResult.getBlockPos().getCenter())+1.0F)*2.0F, 1.0F, false);
