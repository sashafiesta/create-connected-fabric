...
public class CCSoundEvents {
    ...
    public static class SoundEntryProvider implements DataProvider {
        ...
        public SoundEntryProvider(DataGenerator generator) {
            output = generator.vanillaPackOutput;
        }
    ...
    private static class WrappedSoundEntry extends SoundEntry {
        ...
        public void prepare() {
            for (int i = 0; i < wrappedEvents.size(); i++) {
                ...
                compiledEvents.add(new WrappedSoundEntry.CompiledSoundEvent(location, wrapped.volume(), wrapped.pitch()));
            }
        }
        ...
        public void register() {
            for (WrappedSoundEntry.CompiledSoundEvent compiledEvent : compiledEvents) {
                ResourceLocation location = compiledEvent.id();
        ...
        public SoundEvent getMainEvent() {
            return BuiltInRegistries.SOUND_EVENT.get(compiledEvents.get(0).id());
        }
        ...
        public void play(Level world, Player entity, double x, double y, double z, float volume, float pitch) {
            for (WrappedSoundEntry.CompiledSoundEvent event : compiledEvents) {
                SoundEvent soundEvent = BuiltInRegistries.SOUND_EVENT.get(event.id());
                if (soundEvent != null)
                    world.playSound(entity, x, y, z, soundEvent, category, event.volume() * volume,
                            event.pitch() * pitch);
            }
        }
        ...
        public void playAt(Level world, double x, double y, double z, float volume, float pitch, boolean fade) {
            for (WrappedSoundEntry.CompiledSoundEvent event : compiledEvents) {
                SoundEvent soundEvent = BuiltInRegistries.SOUND_EVENT.get(event.id());
                if (soundEvent != null)
                    world.playLocalSound(x, y, z, soundEvent, category, event.volume() * volume,
                            event.pitch() * pitch, fade);
            }
        }

        private record CompiledSoundEvent(ResourceLocation id, float volume, float pitch) {
        }
    ...
    private static class CustomSoundEntry extends SoundEntry {
        ...
        @Override
        public void prepare() {
            // No-op on Fabric, registration happens in register()
        }
